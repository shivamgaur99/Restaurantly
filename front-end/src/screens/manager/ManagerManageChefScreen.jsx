...
const ManagerManageChefScreen = (props) => {
  ...
  return (
    ...
    >
      {/* If there is no order data, display a centered message */}
      {error && <h1 className="text-center">No Orders to Assign</h1>}
      {response && response.data.length === 0 && (
        <h1 className="text-center">No Orders to Assign</h1>
      )}
      {response &&
        response.data.length > 0 &&
        response.data.map((sub, index) => {
          return (
            <div className="container p-5 text-white" key={index}>
